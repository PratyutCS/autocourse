...
def extract(file):
    """Extract text from PDF using pdfplumber with enhanced error handling."""
    try:
...
def get_mapping_template(program):
    """Return the appropriate empty mapping template based on program."""
...
def ai(text):
    ...
    # Simplified prompt with reduced instructions
    prompt = f"""
...
Key requirements:
1. Program field must be exactly one of these values (1-10), representing the appropriate academic program.

2. For course_code field: Must be exactly 7 characters (3 letters followed by 4 numbers). If invalid, leave empty.

3. Extract course_name, Module/Semester, Session, and course_description.

4. For Course Syllabus: Extract topic name, related CO, and session count.

5. For Learning Resources: Extract textbooks and reference links as simple strings in arrays.

6. For internalAssessmentData: Extract component name, weightage, evaluation week, and remarks.

7. For courseOutcomes: Extract all course outcomes (CO1, CO2, etc.) with their descriptions.
   - DO NOT EXTRACT the actual COPO mapping values from tables
   - But DO create empty mapping template entries for each course outcome found
   - Each CO should have a corresponding empty entry in mappingData

8. Ensure last_modified field has current date/time in format: "08:30 AM, 22/03/2025".

Length Limits:
- course_name: Max 128 characters
- Module/Semester: Max 10 characters
- Session: Max 12 characters
- course_description: Max 4096 characters
"""
    ...
    try:
        ...
        if cleaned_response:
            # Get program and create empty mapping templates for each CO
            program = cleaned_response.get("Program", 1)
            if "copoMappingData" in cleaned_response:
                if "courseOutcomes" in cleaned_response["copoMappingData"]:
                    # Create mappingData structure with empty mappings for each CO
                    mapping_data = {}
                    for co in cleaned_response["copoMappingData"]["courseOutcomes"]:
                        mapping_data[co] = get_mapping_template(program)
                    
                    # Replace any existing mappingData with our empty templates
                    cleaned_response["copoMappingData"]["mappingData"] = mapping_data

            # Validate course code
            if "course_code" in cleaned_response:
                cleaned_response["course_code"] = validate_course_code(cleaned_response["course_code"])

            # Add timestamp
            cleaned_response["last_modified"] = datetime.now().strftime("%I:%M %p, %d/%m/%Y")
            return json.dumps(cleaned_response, ensure_ascii=False)
            
        # If no cleaned response, just update the template with timestamp
